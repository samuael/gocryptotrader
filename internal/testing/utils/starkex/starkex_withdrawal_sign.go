...
import (
	...
	"github.com/thrasher-corp/gocryptotrader/internal/testing/utils/hash"
)
...
func signWithdraw(param *WithdrawalToAddressParams) (string, error) {
	quantumAmount := decimal.New(int64(param.Amount), 1).Mul(resolutionUsdc).BigInt()
	nonce := big.NewInt(int64(param.Nonce)) // NonceByClientId(param.Nonce)
	...
	// pedersen hash
	cfg, err := hash.LoadPedersenConfig("")
	if err != nil {
		return "", err
	}
