...
func (ap *Apexpro) ProcessOrderSignature(ctx context.Context, arg *CreateOrderParams) (string, string, error) {
	...
	nonceVal, ok := big.NewInt(0).SetString(arg.ClientOrderID, 0)
	if !ok {
		return "", "", fmt.Errorf("%w, assetId: %s", errUserNonceRequired, arg.ClientOrderID)
	}
	...
	newArg := &starkex.CreateOrderWithFeeParams{
		...
		Nonce:                   nonceVal, //nonceFromClientID(arg.ClientOrderID),
...
func (ap *Apexpro) ProcessWithdrawalToAddressSignature(ctx context.Context, arg *WithdrawalToAddressParams) (string, string, error) {
	creds, err := ap.GetCredentials(context.Background())
	if err != nil {
		return "", "", err
	}
...
func (ap *Apexpro) ProcessWithdrawalSignature(ctx context.Context, arg *WithdrawalParams) (string, string, error) {
	creds, err := ap.GetCredentials(context.Background())
	if err != nil {
		return "", "", err
	}
